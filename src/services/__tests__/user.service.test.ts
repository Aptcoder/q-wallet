...
describe('User service', () => {
    ...
    it('Should auth user if password correct', async () => {
        ...
        userService._generateToken = jest.fn().mockImplementation(() => Promise.resolve({ accessToken: 'sampletoken '}))
        const findByEmailSpy = jest.spyOn(mockUserRepository, 'findByEmail').mockImplementation(() => { return Promise.resolve(user)})
        const result = await userService.auth({ email: 'sample@gmail.com', password: 'samplemilz'})
